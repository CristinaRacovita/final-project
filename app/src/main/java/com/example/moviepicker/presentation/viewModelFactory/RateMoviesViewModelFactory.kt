...
class RateMoviesViewModelFactory(private val detailsUseCase: FetchMovieDetailsUseCase,val ratingsUseCase: AddRatingsUseCase, val sharedPreferences: SharedPreferences, val movies: List<DisplayMovieItemViewModel>) : ViewModelProvider.Factory {
    override fun <T : ViewModel?> create(modelClass: Class<T>): T {
        if (modelClass.isAssignableFrom(RateMoviesViewModel::class.java)) {
            return RateMoviesViewModel(detailsUseCase, ratingsUseCase, sharedPreferences, movies) as T
        }
